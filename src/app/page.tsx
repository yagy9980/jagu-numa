...
export default function Home(){
  ...
  function save(e:FormEvent<HTMLFormElement>){e.preventDefault();const d=new FormData(e.currentTarget);const p:Play={id:Date.now(),machine:String(d.get("machine")),shop:String(d.get("shop"))||"店舗未設定",games:Number(d.get("games")),big:Number(d.get("big")),reg:Number(d.get("reg")),coins:Number(d.get("coins")),yen:Number(d.get("yen")),memo:String(d.get("memo"))||"メモなし",date:String(d.get("date"))};const next=[p,...plays];setPlays(next);localStorage.setItem("jagu-numa-plays",JSON.stringify(next));setModal(false);setTab("records");setToast("実戦データを保存しました");}
