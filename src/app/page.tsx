...
const hitWithin=(average:number,chance:number)=>average>1?Math.ceil(Math.log(1-chance)/Math.log(1-1/average)):0;

export default function Home(){
  const [tab,setTab]=useState<Tab>("home");
  const [games,setGames]=useState(6324),[big,setBig]=useState(29),[reg,setReg]=useState(25),[currentSpins,setCurrentSpins]=useState(320);
  ...
  const forecast=useMemo(()=>({chancePerSpin:combined>1?100/combined:0,p50:hitWithin(combined,.5),p70:hitWithin(combined,.7),p90:hitWithin(combined,.9)}),[combined]);
  ...
  return <main className="shell">
    ...
    {tab==="judge"&&<section className="content"><div className="warning"><b>!</b><p>入力した実戦値を整理する参考機能です。設定や次回当選を確定するものではありません。</p></div><div className="inputs"><label>総回転数<input type="number" min="0" value={games} onChange={e=>setGames(Number(e.target.value))}/><span>G</span></label><label>BIG回数<input type="number" min="0" value={big} onChange={e=>setBig(Number(e.target.value))}/><span>回</span></label><label>REG回数<input type="number" min="0" value={reg} onChange={e=>setReg(Number(e.target.value))}/><span>回</span></label><label className="currentSpin">現在のハマり回転数<input type="number" min="0" value={currentSpins} onChange={e=>setCurrentSpins(Number(e.target.value))}/><span>G</span></label></div><div className="rateGrid"><div><small>BB確率</small><b>{show(rate(games,big))}</b></div><div><small>REG確率</small><b>{show(rr)}</b></div><div className="wide"><small>ボーナス合算</small><b>{show(combined)}</b></div></div><div className="judgement"><small>現在の設定期待度</small><strong>{judge.n}</strong><em>{judge.label}</em><div className="meter"><span style={{width:`${judge.pct}%`}}/></div><p>{judge.note}</p></div><div className="forecast"><header><div><small>NEXT PEKA FORECAST</small><h2>次のペカり目安</h2></div><span>🤡</span></header>{combined>1?<><div className="forecastNow"><small>次の1Gで当たる推定確率</small><b>{forecast.chancePerSpin.toFixed(2)}%</b><em>現在 {currentSpins.toLocaleString()}G</em></div><div className="forecastGrid"><div><small>50%到達</small><b>あと約{forecast.p50}G</b><span>{(currentSpins+forecast.p50).toLocaleString()}G付近</span></div><div><small>70%到達</small><b>あと約{forecast.p70}G</b><span>{(currentSpins+forecast.p70).toLocaleString()}G付近</span></div><div><small>90%到達</small><b>あと約{forecast.p90}G</b><span>{(currentSpins+forecast.p90).toLocaleString()}G付近</span></div></div><p>合算 {show(combined)} が今後も続くと仮定した統計的な目安です。ハマり回転数が増えても、次の1Gの当選確率そのものは上がりません。</p></>:<p>BIG・REG回数を入力すると、次の当たり目安を表示します。</p>}</div><button className="primary full" onClick={()=>setModal(true)}>このデータを実戦記録に残す</button><p className="legal">利益を保証するものではありません。公表値、サンプル数、ホール状況と合わせてご利用ください。</p></section>}
