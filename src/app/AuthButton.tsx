...
type Session = {
  ...
  expires_at?: number;
  user: { id: string; email?: string; user_metadata?: { display_name?: string; signup_origin?: string } };
};
...
async function authRequest(path: string, body?: unknown, token?: string, method: "GET" | "POST" | "PUT" = "POST") {
  ...
  if (!response.ok) {
    ...
    throw new Error(message.includes("Invalid login") ? "メールアドレスまたはパスワードが違います。" : message.includes("already registered") ? "このメールアドレスは登録済みです。" : message);
  }
  return result;
}
...
export default function AuthButton() {
    ...
    void (async () => {
      try {
        ...
        if (accessToken) {
          ...
          const expiresAt = Math.floor(Date.now() / 1000) + Number(hash.get("expires_in") || 3600);
          const profile = await authRequest("user", undefined, accessToken, "GET").catch(() => null);
        ...
        if (parsed.expires_at && parsed.expires_at * 1000 <= Date.now()) { localStorage.removeItem(STORAGE_KEY); return; }
        setSession(parsed);
    ...
    try {
      if (mode === "recovery") {
        ...
        if (result.access_token) {
          ...
          setMessage("確認メールを送りました。メール内のリンクを押すと登録元の沼へ戻ります。");
        }
      } else {
  ...
  async function logout() {
    ...
    localStorage.removeItem(STORAGE_KEY); setSession(null); setMessage(""); setOpen(false);
  }
  ...
  const modal = open ? (
    <div className="numa-auth-backdrop" onMouseDown={() => setOpen(false)}>
      <section className="numa-auth-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {session && mode !== "recovery" ? (
          <div className="numa-auth-account">
            <p className="numa-auth-profile"><b>{label}</b><span>{session.user.email}</span><small>1つのNUMA会員IDでジャグ沼・甘デジ沼の両方を利用できます。</small></p>
            <a className="numa-menu-primary" href="/shops">🏢 店舗データを見る</a>
      ...
      <style jsx>{`
        .numa-auth-backdrop{position:fixed;inset:0;z-index:2147483000;background:rgba(0,0,0,.8);display:flex;align-items:center;justify-content:center;padding:max(16px,env(safe-area-inset-top)) 16px max(16px,env(safe-area-inset-bottom));overflow:auto;-webkit-overflow-scrolling:touch}.numa-auth-modal{box-sizing:border-box;width:min(430px,100%);max-height:calc(100dvh - 32px);overflow:auto;background:#071b14;color:#f7f5ef;border:1px solid #8b35a8;border-radius:24px;padding:20px;box-shadow:0 24px 80px #000;position:relative}.numa-auth-modal header{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:14px}.numa-auth-modal header small{color:#d66df2;letter-spacing:.14em;font-size:11px}.numa-auth-modal h2{font-size:26px;line-height:1.2;margin:4px 0 0}.numa-auth-close{flex:0 0 44px;width:44px!important;height:44px!important;border:0!important;background:#ffffff0d!important;color:#ddd!important;border-radius:50%!important;font-size:30px!important;padding:0!important}.numa-auth-guide{margin:0 0 14px;padding:11px 12px;border-radius:12px;background:#181322;color:#e7d7ec;font-size:13px;line-height:1.55}.numa-auth-tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px}.numa-auth-tabs button{width:auto!important;height:auto!important;white-space:nowrap!important;border:1px solid #734083!important;background:#17151c!important;color:#ddd!important;border-radius:12px!important;padding:12px 8px!important;font-size:15px!important;font-weight:800!important}.numa-auth-tabs .active{background:#6e2185!important;color:#fff!important}.numa-auth-modal form{display:grid;gap:14px}.numa-auth-modal label{display:grid;gap:7px;color:#c6d0ca;font-size:15px;font-weight:700}.numa-auth-modal input{width:100%!important;height:52px!important;box-sizing:border-box;border:1px solid #653071;border-radius:12px;background:#04120d;color:#fff;padding:0 14px!important;font-size:16px!important}.numa-auth-submit{width:100%!important;min-height:52px!important;border:0!important;border-radius:12px!important;background:linear-gradient(100deg,#ef3c32,#bd1024)!important;color:#fff!important;padding:14px!important;font-size:16px!important;font-weight:900!important}.numa-auth-reset{display:block!important;width:auto!important;height:auto!important;margin:16px auto 0!important;padding:8px 12px!important;border:0!important;background:transparent!important;color:#d98bf0!important;text-decoration:underline!important;font-size:14px!important}.numa-auth-error,.numa-auth-success{border-radius:12px;padding:12px;margin:0;font-size:13px}.numa-auth-error{background:#401d18;color:#ffaaa3}.numa-auth-success{background:#123426;color:#85e3b0}.numa-auth-account{display:grid;gap:10px}.numa-auth-profile{display:grid;gap:5px;background:#101f19;border-radius:14px;padding:16px;margin:0}.numa-auth-profile b{font-size:21px}.numa-auth-profile span{color:#aab5af;overflow-wrap:anywhere}.numa-auth-profile small{color:#8fa39a;line-height:1.5;margin-top:6px}.numa-menu-primary,.numa-menu-link{display:flex;align-items:center;min-height:52px;padding:0 16px;border-radius:12px;text-decoration:none;font-weight:900}.numa-menu-primary{background:linear-gradient(100deg,#6e2185,#4a1760);color:#fff;border:1px solid #a54cc0}.numa-menu-link{background:#101f19;color:#f5f5f2;border:1px solid #284337}.numa-logout{width:100%!important;min-height:50px!important;border:1px solid #743a3a!important;border-radius:12px!important;background:#2b1515!important;color:#ffb0aa!important;font-weight:800!important}@media(max-width:480px){.numa-auth-backdrop{align-items:flex-start;padding-top:max(18px,env(safe-area-inset-top))}.numa-auth-modal{margin:auto 0;padding:18px;border-radius:20px}}
      `}</style>
    </div>
  ) : null;
